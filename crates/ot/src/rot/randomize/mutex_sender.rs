...
#[derive(Debug)]
pub struct MutexRandomizeRCOTSender<T> {
    core: Core<T>,
}

impl<T> MutexRandomizeRCOTSender<T> {
...
impl<T> ROTSender<[Block; 2]> for MutexRandomizeRCOTSender<T>
where
    T: RCOTSender<Block>,
{
...
#[async_trait]
impl<T> Flush for MutexRandomizeRCOTSender<T>
where
    T: Flush + Send,
{
    type Error = T::Error;
