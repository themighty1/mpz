//! Optimized RNS Slot Multiplication V2 for BGV using WebGPU.
//!
//! This is an optimized version of rns_slot_mul.rs with:
//! - Fused pointwise_mul + inverse_ntt kernel to reduce dispatch count
//! - From 17 dispatches (1 + k + k + 2k) to 9 dispatches (1 + k + k)
//!
//! Pipeline:
//!   slot_encode -> forward_ntt (x k) -> fused_mul_intt (x k)
//!
//! The key optimization: fused_mul_intt combines pointwise multiplication
//! with inverse NTT in a single kernel, processing both c0 and c1.
...
/// GPU context for optimized RNS batched slot multiplication.
///
/// Key optimizations:
/// 1. Fuses pointwise multiplication with inverse NTT (reduces kernel count)
/// 2. Uses workgroup_id.y for mod_idx (reduces submits from 6 to 2 per call)
/// 3. Combined twiddle/params buffers for all k moduli (single bind group)
///
/// Pipeline: slot_encode (1 submit) → forward_ntt + fused_mul_intt (1 submit)
pub struct RnsSlotMulGpuV2 {
    device: Device,
    queue: Queue,
    ...
    // Combined buffers for all k moduli (indexed by mod_idx in shader)
    all_rns_twiddles_buffer: Buffer,      // k * n * 2 * 2 u32s (psi + omega for all k)
    all_rns_inv_twiddles_buffer: Buffer,  // k * n * 2 * 2 u32s (psi_inv + omega_inv for all k)
    all_rns_params_buffer: Buffer,        // k * 8 u32s (ModulusParams for all k)

    // Pre-allocated buffers
    preallocated_encoded_buffer: Buffer,
    preallocated_pt_ntt_buffer: Buffer,   // Now sized for k moduli: max_batches * k * n
    ...
    // Single bind groups (mod_idx comes from workgroup_id.y)
    forward_ntt_bind_group: BindGroup,
    fused_mul_intt_bind_group: BindGroup,
}

impl std::fmt::Debug for RnsSlotMulGpuV2 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RnsSlotMulGpuV2")
...
/// Forward NTT shader - applies twist and forward NTT.
...
mod tests {
    ...
    fn test_v2_gpu_context_creation() {
        ...
        let result = RnsSlotMulGpuV2::new(params);
