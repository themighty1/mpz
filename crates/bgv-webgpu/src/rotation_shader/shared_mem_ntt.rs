...
/// Full NTT in shared memory - twist, bitrev, butterflies, scale all in one kernel.
/// Requires 64KB shared memory for n=8192.
pub const SHARED_MEM_NTT_SHADER: &str = r#"
#import math
...
@group(0) @binding(3) var<storage, read> psi_powers: array<u32>;     // for twist
@group(0) @binding(4) var<storage, read> n_inv: array<u32>;          // n^(-1) mod q for inverse
...
fn shared_mem_ntt(@builtin(local_invocation_id) local_id: vec3<u32>) {
    let tid = local_id.x;
    let n = params.n;
    let log_n = params.log_n;
    let is_inverse = params.is_inverse;
    ...
    // === LOAD: Each thread loads 8 elements, applies twist, stores to bit-reversed position ===
    for (var i = 0u; i < elements_per_thread; i++) {
        ...
        // Apply twist (multiply by psi^idx for forward, psi^(-idx) for inverse)
        let psi_base = idx * 2u;
    ...
    // === STORE: Apply scale (for inverse) and write back ===
    for (var i = 0u; i < elements_per_thread; i++) {
        let idx = tid * elements_per_thread + i;
        ...
        if (is_inverse == 1u) {
            ...
            let prod = math::mul64(val, n_inv_val);
            val = math::barrett_reduce(prod, q, mu0, mu1, mu2, mu3);
        }
