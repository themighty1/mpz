...
import init, * as wasm from '../pkg/mpz_misc_benches.js';
...
async function runBenchmarks(type, filter = null, concurrency = 8) {
    ...
    try {
        const results = await bench.runAllBenchmarks(config.iterations, config.samples, filter, concurrency);
...
async function main() {
    try {
        ...
        console.log('Loading Zig WASM...');
        await bench.initZigWasm();
        console.log('Zig WASM loaded');

        // Load private-memory WASM module
        console.log('Loading Private WASM...');
        await bench.initPrivateWasm();
        console.log('Private WASM loaded');
