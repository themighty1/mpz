...
pub(crate) fn trace_impl(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    trace_fn.sig.inputs.insert(
        0,
        parse_quote!(state: &'trace std::cell::RefCell<::mpz_circuits::BuilderState>),
    );
    ...
    let return_expr: Expr = if let Type::Tuple(tuple_type) = &return_type {
        ...
        parse_quote!((#(::mpz_circuits::Tracer::new(state, output[#i].clone().try_into().unwrap())),*))
    } else {
        parse_quote!(::mpz_circuits::Tracer::new(
            state,
