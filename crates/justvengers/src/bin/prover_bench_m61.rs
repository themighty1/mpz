//! Prover-only benchmark binary using M61 Mersenne prime field.
//!
//! Loads pre-generated verifier messages and benchmarks only the prover.
//!
//! Usage:
//!   REPS=1000 ITERS=10 cargo run --release --features mersenne --bin prover_bench_m61
//!
//! Requires: verifier_msgs_m61_REPS.bin (generated by verifier_setup_m61)
...
fn run_prover_only<const R: usize>(
    ...
    eprintln!("=== PROVER BENCHMARK (M61) END ===\n");
    eprintln!(
...
fn main() {
    ...
    eprintln!("Using M61 Mersenne prime field (2^61 - 1)");

    let file = File::open(&filename).unwrap_or_else(|_| {
        eprintln!("Error: {} not found. Run verifier_setup_m61 first:", filename);
        eprintln!("  REPS={} cargo run --release --features mersenne --bin verifier_setup_m61", reps);
        std::process::exit(1);
    });
    ...
    eprintln!("\nProver Benchmark Configuration (M61):");
