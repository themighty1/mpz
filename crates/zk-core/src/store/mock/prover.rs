...
impl ProverStore {
    ...
    pub fn set_macs(&mut self, masks: &BitSlice, macs: &[Mac]) -> Result<()> {
        ...
        for range in self.view.flush().commit.iter_ranges() {
            ...
            // CHANGED: no need to adjust MACs.
    ...
    pub fn send_flush(&mut self, transcript: &mut Hasher) -> Result<ProverFlush> {
        ...
        for range in self.view.flush().commit.iter_ranges() {
            ...
            // CHANGED: don't apply masks to data.
            //adjust[i..i + slice.len()] ^= masks;

            i += slice.len();
        }

        // CHANGED: don't update the transcript.
        //let adjust_len = adjust.len();
        //transcript.update(&adjust.as_raw_slice().as_bytes()[..adjust_len.
        // div_ceil(8)]);
