    ...
    pub(crate) fn check_prover(
        ...
        cfg_if! {
            if #[cfg(feature = "rayon")] {
                ...
                let mut u = Block::ZERO;
                let mut v = Block::ZERO;
                let mut rng = rng;  // Shadow as mutable

                for &triple in &macs {
                    let mut chi = Block::ZERO;
                    rng.fill_bytes(chi.as_mut());

                    let (u_part, v_part) = compute_terms(triple, chi);
                    u ^= u_part;
                    v ^= v_part;
                }
            }
        ...
        cfg_if! {
            if #[cfg(feature = "rayon")] {
                ...
                for &triple in &keys {
                    ...
                    let w_part = compute_term(triple, chi, delta);
                    w ^= w_part;
                }
            }
        }
