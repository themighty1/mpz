//! Benchmarks for WRK17 authenticated garbling preprocessing phase.
//!
//! Measures preprocessing throughput (AND gates/sec) for the authenticated garbling protocol.
//! This includes generate_pre_1 through generate_pre_4, generate_free, and generate_de.
//!
//! Run with: `cargo bench -p mpz-garble-core --bench wrk17_preprocess`

use criterion::{BenchmarkId, Criterion, Throughput, black_box, criterion_group, criterion_main};
use mpz_circuits::AES128;
use mpz_garble_core::{AuthGen, bit_shares_from_cot, SSP};
...
fn bench_wrk17_preprocess(c: &mut Criterion) {
    let mut group = c.benchmark_group("wrk17_preprocess");
    ...
    for &(threshold, name) in THRESHOLDS {
        ...
        group.bench_function(BenchmarkId::new("preprocess", name), |b| {
            b.iter(|| {
                for _ in 0..iterations {
                    let mut gb = AuthGen::new(0, bucket_size);
                    let mut ev = AuthGen::new(0, bucket_size);

                    // Measure all preprocessing steps
                    ...
                    // Black box the outputs to prevent optimization
                    black_box((px_gen, py_gen, px_eval, py_eval));
                }
            })
        });
    }
...
criterion_group!(benches, bench_wrk17_preprocess);
