...
//!
//! Measures garbler throughput (AND gates/sec) for the authenticated garbling protocol.
//! Preprocessing is done beforehand and not included in timing.
//!
//! Run with: `cargo bench -p mpz-garble-core --bench wrk17_garble`

use criterion::{BenchmarkId, Criterion, Throughput, black_box, criterion_group, criterion_main};
use mpz_circuits::AES128;
use mpz_garble_core::{AuthGen, bit_shares_from_cot, Key, SSP};
    ...
    for &(threshold, name) in THRESHOLDS {
        ...
        group.bench_function(BenchmarkId::new("iter", name), |b| {
            b.iter(|| {
                ...
                let mut ev = AuthGen::new(0, bucket_size);  // Mock evaluator for generating px/py

                // Full preprocessing protocol
                let (c_gen, mut g_gen) = gb.generate_pre_1(circuit, delta_a, &gen_input_shares, &gen_and_shares).unwrap();
                let (c_eval, mut g_eval) = ev.generate_pre_1(circuit, delta_b, &eval_input_shares, &eval_and_shares).unwrap();

                let gr_gen = g_eval.clone();
                let gr_eval = g_gen.clone();

                let d_gen = gb.generate_pre_2(delta_a, c_gen, &mut g_gen, gr_gen).unwrap();
                let d_eval = ev.generate_pre_2(delta_b, c_eval, &mut g_eval, gr_eval).unwrap();

                let dr_gen = d_eval.clone();
                let dr_eval = d_gen.clone();

                let data_gen = gb.generate_pre_3(delta_a, &mut g_gen, d_gen, dr_gen).unwrap();
                let data_eval = ev.generate_pre_3(delta_b, &mut g_eval, d_eval, dr_eval).unwrap();
                ...
                // Measure garbling phase only
                let masked_values = vec![false; circuit.feed_count()];
                for _ in 0..iterations {
                    let mut iter = gb.generate(circuit, delta_a, &input_keys, px_eval.clone(), py_eval.clone()).unwrap();
        ...
        group.bench_function(BenchmarkId::new("batched", name), |b| {
            b.iter(|| {
                ...
                let masked_values = vec![false; circuit.feed_count()];
                for _ in 0..iterations {
                    let mut iter = gb.generate_batched(circuit, delta_a, &input_keys, px_eval.clone(), py_eval.clone()).unwrap();
                    let _: Vec<_> = iter.by_ref().collect();
                    black_box(iter.finish(masked_values.clone()).unwrap());
                }
            })
