    ...
    I: Iterator<Item = &'a Gate>,
{
    ...
    pub fn finish(mut self) -> Result<GarblerOutput, GarblerError> {
        ...
        // Return only the 0-label for each output wire.
...
fn and_gate(
    ...
    //
    // Compute the R matrix index for both sample_r_odd lookup and
    // apply_r_to_inputs. Index = (pi_a << 3) | (pi_b << 2) | (r0 << 1) | r1
...
///
/// For AND gates, the identity block (true output) is at position (!pi_a,
/// !pi_b).
fn solve_for_output(
    rhs: &[[u8; 8]; 8],
    delta: &SlicedLabel,
