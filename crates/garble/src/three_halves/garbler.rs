use std::sync::Arc;

use mpz_circuits::Circuit;
use mpz_common::Context;
use mpz_garble_core::three_halves::{Garbler as GarblerCore, GarblerOutput};
use mpz_memory_core::correlated::{Delta, Key};
use rand::{SeedableRng, rngs::StdRng};
use serio::SinkExt;

/// Generate a garbled circuit, streaming the encrypted gates to the evaluator
/// in batches.
///
/// # Blocking
///
/// This function performs blocking computation, so be careful when calling it
/// from an async context.
///
/// # Arguments
///
/// * `ctx` - The context to use.
/// * `circ` - The circuit to garble.
/// * `delta` - The garblers delta value.
/// * `inputs` - The inputs of the circuit.
/// * `seed` - The seed for randomness.
#[tracing::instrument(fields(thread = %ctx.id()), skip_all)]
pub(crate) async fn generate(
    ctx: &mut Context,
    circ: Arc<Circuit>,
    delta: Delta,
    inputs: &[Key],
    seed: [u8; 32],
) -> Result<GarblerOutput, GarblerError> {
    let mut gb = GarblerCore::default();
    let mut rng = StdRng::from_seed(seed);
    let mut gb_iter = gb.generate_batched(&circ, delta, inputs, &mut rng)?;
    let io = ctx.io_mut();

    while let Some(batch) = gb_iter.by_ref().next() {
        io.feed(batch).await?;
    }
    io.flush().await?;

    Ok(gb_iter.finish()?)
}

/// Garbler error.
#[derive(Debug, thiserror::Error)]
#[error(transparent)]
pub(crate) struct GarblerError(#[from] ErrorRepr);

#[derive(Debug, thiserror::Error)]
enum ErrorRepr {
    #[error("core error: {0}")]
    Core(#[from] mpz_garble_core::three_halves::GarblerError),
    #[error("io error: {0}")]
    Io(#[from] std::io::Error),
}

impl From<std::io::Error> for GarblerError {
    fn from(err: std::io::Error) -> Self {
        GarblerError(ErrorRepr::Io(err))
    }
}

impl From<mpz_garble_core::three_halves::GarblerError> for GarblerError {
    fn from(err: mpz_garble_core::three_halves::GarblerError) -> Self {
        GarblerError(ErrorRepr::Core(err))
    }
}
