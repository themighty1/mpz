...
use mpz_garble_core::three_halves::{EvaluatorOutput, GarbledCircuit, evaluate_garbled_circuits};
...
use crate::{
    three_halves::evaluator, protocol::semihonest::take_preprocess_calls,
...
async fn evaluate<COT>(
    ...
    let EvaluatorOutput {
        outputs: output_macs,
    } = evaluator::evaluate(ctx, circ, &input_macs)
        .await
        .map_err(VmError::execute)?;
