...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    for &(threshold, name) in THRESHOLDS {
        ...
        println!("Recording for garbler ({})...", name);
        ...
        group.bench_function(BenchmarkId::new("st", name), |b| {
            b.iter(|| {
                block_on(async {
                    let mut ctx =
                        replay_st_context(recorded.clone(), max_frame_length(&circuit_clone, circuit_count));
                    run_garbler_with_replay(&mut ctx, circuit_clone.clone(), circuit_count, delta).await;
                })
            });
        });
    }
    ...
    for &(threshold, name) in THRESHOLDS {
        ...
        println!("Recording for MT garbler ({})...", name);
        ...
        group_mt.bench_function(BenchmarkId::new("mt", name), |b| {
            b.iter(|| {
                block_on(async {
                    ...
                    run_garbler_with_replay_mt(&mut exec, circuit_clone.clone(), circuit_count, delta_mt).await;
                })
            });
        });
    }
