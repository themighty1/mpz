//! Garbler benchmarks with message replay.
//!
//! Records evaluator->garbler messages once, then benchmarks
//! garbler execution using replay contexts.
...
#[cfg(target_arch = "wasm32")]
async fn record_for_garbler(circuit_count: usize, seed: u64, concurrency: usize) -> RecordedMtData {
    ...
    run_protocol_record_evaluator(&mut exec_gb, &mut exec_ev, circuit_count, seed).await;
...
#[cfg(target_arch = "wasm32")]
#[wasm_bindgen]
pub async fn garble_three_halves_garbler(n: u32, batch_size: u32, concurrency: u32) -> BenchResult {
    ...
    let recorded = record_for_garbler(circuit_count, 0, concurrency as usize).await;
    yield_to_browser().await;
    ...
    for _ in 0..n {
        ...
        run_garbler_with_replay(&mut exec, circuit_count, delta).await;
