...
#[cfg(target_arch = "wasm32")]
async fn record_for_verifier(seed: u64, circuit_count: usize, concurrency: usize) -> (RecordedMtData, Block, Delta) {
...
pub async fn zk_verifier(n: u32, gate_count: u32, concurrency: u32) -> BenchResult {
    ...
    let _handle = web_spawn::spawn(move || {
        let bench_result = pollster::block_on(async {
            ...
            let (recorded, ot_seed, delta) = record_for_verifier(0, circuit_count, concurrency as usize).await;
