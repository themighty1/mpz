//! Isolated ZK prover benchmark for WASM.
//!
...
#[cfg(target_arch = "wasm32")]
async fn record_for_prover(seed: u64, circuit_count: usize, concurrency: usize) -> RecordedMtData {
    ...
    run_protocol_record_verifier(&mut exec_p, &mut exec_v, seed, circuit_count).await;
...
/// * `gate_count` - Target number of AND gates (e.g., 100000, 1000000, 10000000)
/// * `concurrency` - Number of worker threads for parallel execution
#[cfg(target_arch = "wasm32")]
#[wasm_bindgen]
pub async fn zk_prover(n: u32, gate_count: u32, concurrency: u32) -> BenchResult {
    ...
    let _handle = web_spawn::spawn(move || {
        let bench_result = pollster::block_on(async {
            ...
            let recorded = record_for_prover(0, circuit_count, concurrency as usize).await;
            ...
            for _ in 0..n {
                ...
                run_prover_with_replay(&mut exec, circuit_count).await;
